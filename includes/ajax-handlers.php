...
function chatwidgetflowfunnel_verify_number_handler() {
	if ( ! check_ajax_referer( 'verify_chatwidgetflowfunnel_number', 'nonce', false ) ) {
		wp_send_json_error( array( 'message' => __( 'Invalid security token.', 'chat-widget-flowfunnel' ) ) );
	}

	$number = isset( $_POST['number'] ) ? sanitize_text_field( wp_unslash( $_POST['number'] ) ) : '';

	if ( empty( $number ) ) {
		wp_send_json_error( array( 'message' => __( 'Please provide a valid chat widget number.', 'chat-widget-flowfunnel' ) ) );
	}
	...
	if ( $verified ) {
		...
		wp_send_json_success( array( 'message' => __( 'Number verified successfully.', 'chat-widget-flowfunnel' ) ) );
	} else {
		wp_send_json_error( array( 'message' => __( 'Could not verify chat widget number.', 'chat-widget-flowfunnel' ) ) );
	}
}
